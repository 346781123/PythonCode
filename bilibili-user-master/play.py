...
def getViewInfo(url, fpath):
    ...
    try:
        ...
        number = (json_data['data']['play_online'])
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute('INSERT INTO bilibili_play_online(play_online, time) \
            VALUES ("%s","%s")'
                        %
                        (number, nowTime))
            conn.commit()
...
def main():
    ...
    while 1:
        ...
        # 文件路径
        output_path = "F:\\PythonWork\\bilibili-user-master\\bilibiliplay.txt"
